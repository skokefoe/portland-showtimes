...
class BagdadScraper(BaseScraper):
    """Scraper for Bagdad Theater & Pub (McMenamins)."""
    ...
    def fetch_showtimes(self, start_date: datetime, num_days: int = 7) -> List[Dict[str, Any]]:
        ...
        try:
            # McMenamins theaters usually have /events or /movies section
            url = f"{self.theater_url}/events" if not self.theater_url.endswith('/events') else self.theater_url
            ...
            movie_containers = soup.find_all(['div', 'article'], class_=lambda x: x and any(
                keyword in str(x).lower() for keyword in ['movie', 'film', 'event']
            ))
            ...
            for container in movie_containers:
                try:
                    ...
                    print(f"Error parsing movie in Bagdad: {e}")
                    continue

        except Exception as e:
            print(f"Error fetching Bagdad showtimes: {e}")
